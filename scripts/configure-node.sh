...
config_done() {
  ...
  echo "Couchbase Admin UI: http://localhost:$REST_PORT" \
...
wait_for_uri 200 "http://127.0.0.1:$REST_PORT/ui/index.html"

echo "Setting memory quotas with curl:"
curl_check "http://127.0.0.1:$REST_PORT/pools/default" -d memoryQuota=512 -d indexMemoryQuota=512 -d ftsMemoryQuota=512
echo

echo "Configuring Services with curl:"
curl_check "http://127.0.0.1:$REST_PORT/node/controller/setupServices" -d services=kv%2Cn1ql%2Cindex%2Cfts
echo

echo "Setting up credentials with curl:"
curl_check "http://127.0.0.1:$REST_PORT/settings/web" -d port=$REST_PORT -d username=Administrator -d password=password
echo

echo "Enabling memory-optimized indexes with curl:"
curl_check -u Administrator:password -X POST "http://127.0.0.1:$REST_PORT/settings/indexes" -d 'storageMode=memory_optimized'
echo

echo "Loading travel-sample with curl:"
curl_check -u Administrator:password -X POST "http://127.0.0.1:$REST_PORT/sampleBuckets/install" -d '["travel-sample"]'
echo

wait_for_uri 200 "http://127.0.0.1:$REST_PORT/pools/default/buckets/travel-sample" -u Administrator:password
